...
for c in cs:
    ...
    x[0] = [-r1, 0.0, 0.0, r2, 0.0, 0.0, 0.5*d_jup - r1 + c*d_jup, 0.5*np.sqrt(3)*d_jup + c*d_jup, 0.0, 0.5*d_jup - r1 + c*d_jup, -0.5*np.sqrt(3)*d_jup - c*d_jup, 0.0]
    ...
    xprime = np.zeros([nsteps, 2*nbodies])
    xdiff = np.zeros([nsteps, 3*nbodies])
    ...
    plt.title("Newtonian Coordinates")
    plt.axis('equal')
    plt.xlabel("x (meters)")
    plt.ylabel("y (meters)")
    '''
