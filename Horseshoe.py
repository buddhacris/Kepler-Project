...
dv = np.empty([nsteps])
m = [m_sun, m_jup, 1e-6*m_jup] #masses of bodies
...
x[0] = [-r1, 0.0, 0.0, r2, 0.0, 0.0, -1.00001*r2, 0.0, 0.0]
v[0] = [0.0, v_sun, 0.0, 0.0, v0, 0.0, 0.0, -1.000005*v0, 0.0]
x0 = x[0, 3]
...
for i in range(0, len(x)):
    t = dt*i
    xprime[i] = np.array([np.cos(w*t)*xdiff[i, 0] + np.sin(w*t)*xdiff[i, 1], -1*np.sin(w*t)*xdiff[i, 0] + np.cos(w*t)*xdiff[i, 1], np.cos(w*t)*xdiff[i, 3] + np.sin(w*t)*xdiff[i, 4], -1*np.sin(w*t)*xdiff[i, 3] + np.cos(w*t)*xdiff[i, 4], np.cos(w*t)*xdiff[i, 6] + np.sin(w*t)*xdiff[i, 7], -1*np.sin(w*t)*xdiff[i, 6] + np.cos(w*t)*xdiff[i, 7]])
